...
if [[ "${libtype}" == "DNA" ]]; then
    ...
    STAR  --runThreadN 6 --genomeDir ${genome_ref} --readFilesIn ${pfx}_BC_cov_trimmed_trimmed_trimmed.fq.gz --readFilesCommand zcat --outFileNamePrefix ${pfx}_RNA --outSAMtype BAM Unsorted
    samtools view -h -F 256 ${pfx}_RNA_Aligned.out.bam -b > ${pfx}\_RNA\_clean.bam
    samtools sort ${pfx}\_RNA\_clean.bam ${pfx}_RNA_sorted
    $reachtools_bin rmdup2 ${pfx}\_RNA\_sorted.bam
    $reachtools_bin bam2Mtx2 ${pfx}\_RNA\_sorted_rmdup.bam ${transcriptome}
    # cleanup
    ...
    rm "${pfx}_RNA_Aligned.out.bam" "${pfx}_RNA_clean.bam" "${pfx}_RNA_sorted.bam"
else
    echo "Impossible!"
    exit 1
fi
